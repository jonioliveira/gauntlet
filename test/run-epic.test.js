...
// GAUNTLET_FAKE_LINEAR points the runner at a JSON fixture instead of the real CLI,
...
const dry = (args, fixture = FIX) =>
  $`bash bin/run-epic.sh ${{ raw: args }}`
    .env({ ...process.env, GAUNTLET_FAKE_LINEAR: fixture }).text()
...
test("a dependent task lands in a later wave than its blocker", async () => {
  ...
  expect(waves[0]).toContain("gauntlet-JON-2")
  expect(waves[0]).toContain("gauntlet-JON-5")
  expect(waves[0]).not.toContain("gauntlet-JON-3")
  expect(waves[1]).toContain("gauntlet-JON-3")
})

test("a done task is never planned", async () => {
  const out = await dry("JON-1 --dry-run --max-parallel 3")
  expect(out).not.toContain("gauntlet-JON-4")
})
...
test("dry-run prints the pane creation herdr agent start requires", async () => {
  const out = await dry("JON-1 --dry-run --max-parallel 3")
  expect(out).toContain("herdr tab create --cwd <worktree> --label gauntlet-JON-2 --no-focus")
  expect(out).toContain("herdr agent start gauntlet-JON-2 --kind claude --pane <pane-id>")
})
...
test("a task that fails is not relaunched for the rest of the run", async () => {
  ...
  const r = await $`bash bin/run-epic.sh ZZZ-9000 --max-parallel 3`
    .env({ ...process.env, PATH: `${dir}:${process.env.PATH}`, GAUNTLET_FAKE_LINEAR: fixture })
    .nothrow().quiet()
...
function liveStub({ worktreeJson, tabJson, agentStatus }) {
  const dir = mkdtempSync(join(tmpdir(), "gauntlet-live-"))
  const log = join(dir, "calls.log")
...
test("a pane is created in the task's worktree and handed to herdr agent start", async () => {
  ...
  expect(calls).toContain(`herdr tab create --cwd ${s.wt} --label gauntlet-ZZZ-9001 --no-focus`)
  expect(calls).toContain("herdr agent start gauntlet-ZZZ-9001 --kind claude --pane w9:p1")
...
function fetchStub(issuesJson) {
  const dir = mkdtempSync(join(tmpdir(), "gauntlet-fetch-"))
  writeFileSync(join(dir, "orca"), `#!/bin/sh\necho '${issuesJson}'\nexit 0\n`)
...
test("the manifest's edges become blockedBy against Linear's live states", async () => {
  ...
  expect(waves.length).toBe(2)
  expect(waves[0]).toContain("gauntlet-ZZZ-9001")
  expect(waves[1]).toContain("gauntlet-ZZZ-9002")
})
